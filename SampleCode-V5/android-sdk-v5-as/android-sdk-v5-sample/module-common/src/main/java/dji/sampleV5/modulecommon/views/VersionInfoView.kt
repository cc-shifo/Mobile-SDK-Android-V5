...
class VersionInfoView @JvmOverloads constructor(
    ...
    // 可预览1
    // private lateinit var versionInfoVm: VersionInfoVm
    // 可预览2
    private lateinit var versionInfoVm: VersionInfoVm
    ...
    init {
        // LayoutInflater.from(context).inflate(R.layout.layout_version_info, this, true)
    ...
    override fun onAttachedToWindow() {
        ...
        // 可预览1
        // versionInfoVm = ViewModelProvider(context as ComponentActivity)[VersionInfoVm::class.java]
        // 可预览2
        versionInfoVm = (context as ComponentActivity).viewModels<VersionInfoVm>().value
        ...
        versionInfoVm.listenLatestVersionInfo().observeOn(AndroidSchedulers.mainThread())
            .subscribe({
    ...
    private fun refreshCurrentVersionInfo(currentVersionInfo: VersionInfo) {
        ...
        showVersionInfoTo(
            item_current_version_info,
            currentVersionInfo,
            R.string.release_note_tile_current
        )
    }
    ...
    private fun refreshLatestVersionInfo(latest: VersionInfo, latestGreaterThan: Boolean) {
        ...
        item_latest_version_info.setOnClickListener {
            DjiSharedPreferencesManager.putString(
                context,
                SP_KEY_CLICKED_LATEST_VERSION_INFO_STRING,
                latest.versionName
            )
            showVersionInfoTo(
                item_latest_version_info,
                latest,
                R.string.release_note_tile_latest,
                !isClickedLatestVersionInfo(latest)
            )

            DialogUtil.showTwoButtonDialog(context,
                StringUtils.getResStr(R.string.news_dialog_content_tips),
    ...
    private fun showVersionInfoTo(
        itemNewsLayout: ItemNewsLayout,
        ...
        itemNewsLayout.apply {
            ...
            setDate(
                SimpleDateFormat(
                    "yyyy-MM-dd",
                    Locale.getDefault()
                ).format(Date(versionInfo.releaseTimeStamp * 1000))
            )
    ...
    private fun isClickedLatestVersionInfo(latest: VersionInfo): Boolean {
        val clickedVersionName = DjiSharedPreferencesManager.getString(
            context,
            SP_KEY_CLICKED_LATEST_VERSION_INFO_STRING,
            ""
        )
    ...
    companion object {
        private const val SP_KEY_CLICKED_LATEST_VERSION_INFO_STRING =
            "sp_key_clicked_latest_version_info_string"
    }
