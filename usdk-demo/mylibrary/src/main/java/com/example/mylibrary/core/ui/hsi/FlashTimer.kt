package com.example.mylibrary.core.ui.hsi
...
object FlashTimer {
    ...
    private fun startFlash() {
        ...
        compositeDisposable.add(Observable.zip(valueObservable, intervalObservable) { value, _ ->
            ...
            for (element in listenerList) element.onValue(value)
            // for (Listener action : listenerList)
            // listenerList.forEach { it.onValue(value) }
        })
    }

    private fun <T> forEach(list: MutableList<Listener>, value: Boolean): Unit {
        for (element in list) element.onValue(value)
    }
